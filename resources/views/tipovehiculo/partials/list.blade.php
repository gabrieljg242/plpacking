...
<table class="table table-bordered data-table display responsive nowrap" style="width:100%">
    ...
    <tbody>
        @foreach($tipovehiculos as $key => $tipovehiculo)
        <tr class="text-center">
            <td>{{ ($key + 1) }}</td>
            <td>{{ $tipovehiculo->nombre }}</td>
            <td>
                @can('tipovehiculo.update')
                <a href="{{ route('tipovehiculo.edit', encrypt($tipovehiculo->id)) }}" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="top" title="Ver detalle"><i class="fas fa-eye fa-fw"></i></a>
                @endcan
            </td>
        </tr>
        @endforeach
...
<script>
    $(document).ready(function() {
        var table = $('.data-table').DataTable( {
            ...
            buttons: [
                @can('tipovehiculo.create')
                    {
                        text: 'Crear Tipo Vehiculo',
                        className: 'btn btn-sm pl-btn-secondary',
                        action: function ( e, dt, node, config ) {
                            location.href = '{{ url("tipovehiculo/create") }}';
                        }
                    },
                @endcan
            ]
